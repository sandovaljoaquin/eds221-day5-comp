...
# Creating an empty matrix to store population sizes 

proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# Populating the first row with year 0 population sizes

proj_year[1, ] <- fish_year0

# Estimates population size for each year based on population size in the previous year 

# 'i in 2' starts estimating population in year 2 
...
#Update column names for population stages
...
proj_df <- data.frame(time_yr, proj_year) %>% 
  #convert format from wide to lomg
